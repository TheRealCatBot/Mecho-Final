...
class TicTacToe : AppCompatActivity(), View.OnClickListener {
    ...
    private fun init() {
        ...
        data class User(val username: Int? = null, val email: Int? = null) {
            // Null default values create a no-argument default constructor, which is needed
            // for deserialization from a DataSnapshot.
        }
        fun writeNewUser(userId: String, flappy: Int, snake: Int) {
            val user = User(flappy, snake)

            database.child("users").child(userId).setValue(user)
        }
        writeNewUser("usr", 0, mecho)
    ...
    private fun check() {
        ...
        if (winnerPlayer != 0) {
            if (winnerPlayer == 1) {
                ...
                mecho += 1;
                database.child("users").child("usr").setValue(mecho)
